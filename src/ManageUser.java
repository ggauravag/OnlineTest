...
public class ManageUser extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3901479648885333063L;
	String userid = null;
	int index = 0;
		...
		if (session != null && session.getAttribute("adminid") != null) {
			...
			try {
				...
				if (rs.next() && rs.getString(3).equals("admin")) {
					query = "select * from user where userid like 'stu%'";
					ps = con.prepareStatement(query);
					rs = ps.executeQuery();
					while (rs.next()) {
						users.add(new User(rs.getString(1), rs.getString(2), rs
